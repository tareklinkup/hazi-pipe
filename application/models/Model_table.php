...
class Model_Table extends CI_Model{
    ...
    public function get_branch_wise_sale_invoice(){
		...
		$res = $this->db->query("SELECT branch_wise_sort_id FROM tbl_salesmaster where SaleMaster_branchid=? order by SaleMaster_SlNo DESC LIMIT 1",$branch_id);

		if($res->num_rows() != 0){
			return $res->row()->branch_wise_sort_id;
		} else {
			return 0;
		}
	}
    ...
    public function generateSalesInvoice(){
        ...
        // }
        $branch_id = $this->session->userdata('BRANCHid');
        $generateBranchWiseSortId = $this->get_branch_wise_sale_invoice() + 1;
        $invoice = 'B'.$branch_id.'-000'.$generateBranchWiseSortId;
		return $invoice;
    }
    ...
    public function get_branch_wise_purchase_invoice(){
        $branch_id = $this->session->userdata('BRANCHid');
		$res = $this->db->query("SELECT branch_wise_sort_id FROM tbl_purchasemaster where PurchaseMaster_BranchID=? order by PurchaseMaster_SlNo DESC LIMIT 1",$branch_id);
    ...
    public function generatePurchaseInvoice(){
        ...
        // }
        $branch_id = $this->session->userdata('BRANCHid');
        $generateBranchWiseSortId = $this->get_branch_wise_purchase_invoice() + 1;
        $invoice = 'B'.$branch_id.'-000'.$generateBranchWiseSortId;
   ...
   public function getEmployeeSalesSummary($clauses = "", $date = null, $dateFrom = null, $dateTo = null) {
        $branchId = $this->session->userdata('BRANCHid');
        
        $saleReports = $this->db->query("
            SELECT 
                Employee_ID,
                Employee_SlNo,
                Employee_Name,
                ...
                SUM(cashReceived) as cashReceived,
                SUM(paidOutAmount) as paidOutAmount,
            ...
            FROM(
                select
                    ...
                    e.previous_due,
                    (
                        select ifnull(sum(sm.SaleMaster_TotalSaleAmount), 0.00) ". ($date == null ? "" : " + ifnull(c.previous_due, 0)") . "
                        from tbl_salesmaster sm 
                        where sm.SalseCustomer_IDNo = c.Customer_SlNo
                        and sm.Status = 'a'
                        " . ($dateTo == null ? "" : " and sm.SaleMaster_SaleDate BETWEEN '$dateFrom' and '$dateTo'") . "
                        " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
                    ) as billAmount,
                    ...
                    (
                        ...
                        and cp.CPayment_status = 'a'
                        " . ($dateTo == null ? "" : " and cp.CPayment_date BETWEEN '$dateFrom' and '$dateTo'") . "
                        " . ($date == null ? "" : " and cp.CPayment_date < '$date'") . "
                    ) as cashReceived,
                    ...
                    (
                        ...
                        " . ($dateTo == null ? "" : " and sr.SaleReturn_ReturnDate BETWEEN '$dateFrom' and '$dateTo'") . "
                    ...
                    join tbl_employee e on e.Employee_SlNo = c.employee_id
                    
                    where c.Customer_brunchid = $branchId
                    
                    and e.Employee_brinchid = $branchId
                    
                    $clauses 
                ) as tbl 
                GROUP BY Employee_SlNo
            ")->result();
    ...
    public function save_data_documents($table, $data){       
        $result= $this->db->insert($table, $data);
