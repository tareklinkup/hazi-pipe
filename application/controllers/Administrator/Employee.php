...
class Employee extends CI_Controller
{
    ...
    public function empSaleRecord(){
           $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = 'Customer Due';
        $data['content'] = $this->load->view('Administrator/due_report/employee_sale_record', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function getEmpSaleRecord(){
        $search_type =  $this->input->post('search_type');
        $startdate =  $this->input->post('startdate');
        $enddate =  $this->input->post('enddate');
        if ($search_type=='all') {
           $datas['result'] = $this->db->query("select e.*,sm.*,sum(sm.SaleMaster_TotalSaleAmount) as SaleMaster_TotalSaleAmount,sum(sm.SaleMaster_DueAmount) as SaleMaster_DueAmount
FROM tbl_employee as e
INNER JOIN tbl_salesmaster as sm 
ON sm.employee_id = e.Employee_SlNo

GROUP BY e.Employee_SlNo")->result();

        }else{
           $datas['result'] = $this->db->query("select e.*,sm.*,sum(sm.SaleMaster_TotalSaleAmount) as SaleMaster_TotalSaleAmount,sum(sm.SaleMaster_DueAmount) as SaleMaster_DueAmount
FROM tbl_employee as e
INNER JOIN tbl_salesmaster as sm 
ON sm.employee_id = '$search_type'

WHERE e.Employee_SlNo='$search_type' AND  e.status='a' AND   sm.SaleMaster_SaleDate BETWEEN  '$startdate' AND '$enddate'

               ")->result();
        }
        $this->load->view('Administrator/due_report/get_employee_sale_record',$datas);
    ...
    public function getEmployees(){
        $employees = $this->db->query("
            ...
            where e.status = 'a' and  e.Employee_brinchid=?
    ...
    public function employee_insert()
    {
        ...
        $data['previous_due'] = $this->input->post('previous_due', true);
        $data['status'] = 'a';
        ...
        // Employee 
        $documents = $this->image_upload($_FILES['documents']);
        $data['documents'] = $documents;
        /// End
                $data['edu_qua'] = $this->input->post('ec_edu');



        //echo "<pre>";print_r($data);exit;
        $last_id = $this->mt->save_data_documents('tbl_employee', $data);
    ...
    $extention_img = explode('.', $file['name']);
    $newNameImg = rand().'.'.$extention_img[1];
    $imgDestination = 'uploads/employeePhoto_thum/'.$newNameImg;
