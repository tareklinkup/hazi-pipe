...
class Sales extends CI_Controller {
    ...
    public function addSales(){
        ...
        try{
            ...
            $generateBranchWiseSortId = $this->mt->get_branch_wise_sale_invoice() + 1;
            ...
            $sales = array(
                ...
                'branch_wise_sort_id'=>$generateBranchWiseSortId
            );
            ...
            foreach($data->cart as $cartProduct){
                $saleDetails = array(
                    ...
                    'SaleDetails_TotalQuantity' => $cartProduct->quantity,
                     'total_size' => $cartProduct->totalSize,
...
//    }
    ...
    public function employeeSaleReport(){
        $data['title'] = "Employee sale report";
        $data['content'] = $this->load->view('Administrator/sales/employee_sale_report', $data, true);
        $this->load->view('Administrator/index', $data);
    }

    public function getEmployeeSaleReport(){
        $data = json_decode($this->input->raw_input_stream);
        $clauses = "";
        $dateFrom = null;
        $dateTo = null;

        if(isset($data->employeeId) && $data->employeeId != "All"){
        ...
        $sales = $this->mt->getEmployeeSalesSummary($clauses, null, $dateFrom, $dateTo);
        $sales = array_map(function($sale) use($dateFrom){
            $sale->previous_date_due = $this->mt->getEmployeeSalesSummary(" and e.Employee_SlNo = '$sale->Employee_SlNo'", $dateFrom)[0]->dueAmount;
            return $sale;
        }, $sales);
