...
class Products extends CI_Controller {
    ...
    public function re_order_list(){
        ...
        $data['productCode'] = $this->mt->generateProductCode();
        $data['content'] = $this->load->view('Administrator/stock/re_order_list', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getTotalStock(){
        ...
        $stock = $this->db->query("
            ...
            left join tbl_brand b on b.brand_SiNo = p.brand
            left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
            where p.status = 'a' and p.is_service = 'false' and p.Product_branchid = '$branchId' $clauses
        ")->result();
