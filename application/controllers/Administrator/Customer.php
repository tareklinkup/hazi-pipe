...
class Customer extends CI_Controller
{
    ...
    public function getCustomers(){
        ...
        $customers = $this->db->query("
            select
                c.*,
                d.District_Name,
                concat(c.Customer_Code, ' - ', c.Customer_Name,' - ',c.Customer_Address,' - ',c.owner_name) as display_name,
                e.Employee_Name,
                e.Employee_ID
            from tbl_customer c
    ...
    public function addCustomer()
    {
        ...
        try{
            ...
            //     if($info['mime'] == "jpg" || $info['mime'] == "png" || $info['mime'] == "gif" || $info['mime'] == "jpeg"){

            //         // update query 
            //         ------

            //       // move file
            //       move_uploaded_file($_FILES["image"]["tmp_name"], asasas);
            //     }else{
            //         echo "This image not allow";
            //     }
            // }
            ...
            if(!empty($_FILES['nidImage'])) {
                $nidImage = $customerObj->Customer_Code . "_nid";
                $ext = pathinfo($_FILES['nidImage']['name'], PATHINFO_EXTENSION);
                $nidImage = $nidImage . '.' . $ext;
                move_uploaded_file($_FILES["nidImage"]["tmp_name"], "./uploads/customers/" . $nidImage);

                $this->db->query("update tbl_customer set NID_copy = ? where Customer_SlNo = ?", [$nidImage, $customerId]);
            }
            if(!empty($_FILES['licenseImage'])) {
                $licenseImage = $customerObj->Customer_Code . "_licence";
                $ext = pathinfo($_FILES['licenseImage']['name'], PATHINFO_EXTENSION);
                $licenseImage = $licenseImage . '.' . $ext;
                move_uploaded_file($_FILES["licenseImage"]["tmp_name"], "./uploads/customers/" . $licenseImage);

                $this->db->query("update tbl_customer set Trade_Licence = ? where Customer_SlNo = ?", [$licenseImage, $customerId]);
            }
