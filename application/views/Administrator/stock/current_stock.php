				...
				<table class="table table-bordered" v-if="searchType == 'current'" style="display:none" v-bind:style="{display: searchType == 'current' ? '' : 'none'}">
					<thead>
						<tr>
							...
							<th>Product Name</th>
							<th>Category</th>
							<th v-if="currentBranch.Company_SlNo == 2">Current Quantity</th>
							<th v-else>Current feet</th>
							<th>Stock Value</th>
						</tr>
					</thead>
					<tbody>
					...
					<tfoot>
						<tr>
							...
							<td>{{ 
								stock.reduce((prev,curr) => prev + +curr.current_quantity, 0).toFixed(2)	
							}}</td>
				...
				<table class="table table-bordered" 
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td>
								{{ parseFloat(product.current_quantity).toFixed(2) }} {{ product.Unit_Name }} 
							</td>
